...
if not (mp.get_property("options/lavfi-complex", "") == "") then return end

local visualizer_name_list = {
    "off", "showcqt", "avectorscope", "showspectrum", "showcqtbar", "showwaves"
}
...
local function get_visualizer(name, quality)
    ...
    if name == "showcqt" then
        ...
        return "[aid1] asplit [ao]," ..
                   ...
                   "scale          = linlin:" .. "wfunc          = tukey:" ..
                   "zero_phase     = on:" .. "fft2           = on," ..
                   "showcqt            =" .. "fps            =" .. fps .. ":" ..
                   "size           =" .. w .. "x" .. h .. ":" ..
                   "count          =" .. count .. ":" ..
                   "csp            = bt709:" .. "bar_g          = 2:" ..
                   "sono_g         = 4:" .. "bar_v          = 9:" ..
                   "sono_v         = 17:" .. "axisfile       = data\\\\:'" ..
                   axis_0 .. "':" ..
    ...
    elseif name == "avectorscope" then
        return "[aid1] asplit [ao]," .. "afifo," .. "aformat            =" ..
                   "sample_rates   = 192000," .. "avectorscope       =" ..
                   "size           =" .. w .. "x" .. h .. ":" ..
                   "r              =" .. fps .. "," ..
                   "format             = rgb0 [vo]"

    elseif name == "showspectrum" then
        return "[aid1] asplit [ao]," .. "afifo," .. "showspectrum       =" ..
                   "size           =" .. w .. "x" .. h .. ":" ..
    ...
    elseif name == "showcqtbar" then
        ...
        return "[aid1] asplit [ao]," ..
                   ...
                   "bar_g          = 2:" .. "sono_g         = 4:" ..
                   "bar_v          = 9:" .. "sono_v         = 17:" ..
                   "sono_h         = 0:" .. "axisfile       = data\\\\:'" ..
                   axis_1 .. "':" .. "axis_h         =" .. axis_h .. ":" ..
                   "font           = 'Nimbus Mono L,Courier New,mono|bold':" ..
                   "fontcolor      = 'st(0, (midi(f)-53.5)/12); st(1, 0.5 - 0.5 * cos(PI*ld(0))); r(1-ld(1)) + b(ld(1))':" ..
                   "tc             = 0.33:" .. "attack         = 0.033:" ..
                   "tlength        = 'st(0,0.17); 384*tc / (384 / ld(0) + tc*f /(1-ld(0))) + 384*tc / (tc*f / ld(0) + 384 /(1-ld(0)))'," ..
                   "format             = yuv420p," .. "split [v0]," ..
                   "crop               =" .. "h              =" .. (h - axis_h) /
                   2 .. ":" .. "y              = 0," .. "vflip [v1];" ..
                   "[v0][v1] vstack [vo]"

    elseif name == "showwaves" then
        return "[aid1] asplit [ao]," .. "afifo," .. "showwaves          =" ..
                   "size           =" .. w .. "x" .. h .. ":" ..
                   "r              =" .. fps .. ":" .. "mode           = p2p," ..
                   "format             = rgb0 [vo]"
...
local function visualizer_hook()
    ...
    mp.set_property("options/lavfi-complex",
                    select_visualizer(atrack, vtrack, albumart))
end
...
local function cycle_visualizer()
    ...
    for i = 1, #visualizer_name_list do
        if (visualizer_name_list[i] == opts.name) then
            index = i + 1
            if index > #visualizer_name_list then index = 1 end
            break
        end
    end
